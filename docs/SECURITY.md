...
- Jira Cloud issue creation has no destination idempotency contract exposed by this integration. An analyst-requested retry after an unknown outcome can create a duplicate despite the embedded correlation label. SignalRoom does not yet refresh the external issue's status.
- SignalRoom does not yet provide Teams, email, or SOAR adapters, or destination-native update/delete workflows.
